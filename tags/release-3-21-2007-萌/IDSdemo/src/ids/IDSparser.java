	...
	if (Character.UnicodeBlock.of(IDS.charAt(0)).equals(Character.UnicodeBlock.IDEOGRAPHIC_DESCRIPTION_CHARACTERS)) { //IF始
	   ...
	   //倒過來作
	    ...
	    for (int i = 0; i < 新字.get子組件框().length; i++) { //FOR
	    	新字.get子組件框()[i][0]+=15;
	    	新字.get子組件框()[i][1]+=15;
	    	新字.get子組件框()[i][2]=新字.get子組件框()[i][2] *226 /256;
	    	新字.get子組件框()[i][3]=新字.get子組件框()[i][3]*226 /256;
		} //FOR
...
//    	以十個刻度來算
    	...
    	if (甲.is末級() && 乙.is末級()) { //IF
			//兩個都是末級
    	System.out.println("甲.is末級() && 乙.is末級()");
    		比例[0]=5;
    		if (Math.abs(筆劃差)>=4 ) { //IF
    		    
    			if(筆劃差<0)	比例[0]=4;//甲筆劃比較少 
    			else 比例[0]=6;
    		} //IF
		} //IF
    	else if (甲.is末級() && !乙.is末級()) { //IF
    	System.out.println("甲.is末級() && !乙.is末級()");
    		if (Math.abs(筆劃差)<=2)	比例[0]=5;
    		else 
    		    {
    		    	if (筆劃差>0) { //IF始  甲>乙
    		    	比例[0]=8;
